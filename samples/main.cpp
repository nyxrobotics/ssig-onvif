...
void PTZTest(std::string url)
{
  ...
  home position\nESPACE to stop movement\nESC to quit"); while((c=getch()) != 27){ c = getch(); if(c==87 || c==119){
          PTZ->tiltUp("PTZ", 5);
      }else if(c == 83 || c==115){
          PTZ->tiltDown("PTZ", 5);
      }else if(c == 65 || c==97){
...
int main(int argc, char** argv)
{
  ...
  /*
      ...
      for(int i=0;i<3 ;i++)
          soap_wsdd_Hello(soap, SOAP_WSDD_ADHOC, "soap.udp://239.255.255.250:3702", soap_wsa_rand_uuid(&soap), NULL,
     l_cameraId, l_types, l_scopes, NULL, l_serviceAddr, 1); soap.connect_flags = oldFlags;
   */
  /*
